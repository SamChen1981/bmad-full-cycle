...
**Check if the current session was triggered by an autopilot skill** (user said "开始迁移", "继续迁移", "开始开发", "start autopilot", "自动执行", or similar). If yes, set `{autopilot_mode}` = `true`. In autopilot mode:
- **SKIP ALL HALT/ASK/CHECKPOINT instructions** in this file
- **Automatically select the story in `review` status** (do not ask the user)
- **Automatically use uncommitted changes or branch diff** as the diff source
- **Automatically use the story file as `{spec_file}`** (set `{review_mode}` = `"full"`)
- **Do NOT present summary or wait for confirmation** — proceed directly to step-02
...
1. **Detect review intent from invocation text.** Check the triggering prompt for phrases that map to a review mode:
   ...
   - **If no match from invocation text, check sprint tracking.** Look for a sprint status file (`*sprint-status*`) in `{implementation_artifacts}` or `{planning_artifacts}`. If found, scan for any story with status `review`. Handle as follows:
     - **Exactly one `review` story:** Set `{story_key}` to the story's key (e.g., `1-2-user-auth`). **If `{autopilot_mode}` = true, automatically confirm and proceed without asking.** Otherwise, suggest it: "I found story {{story-id}} in `review` status. Would you like to review its changes? [Y] Yes / [N] No, let me choose". If confirmed, use the story context to determine the diff source (branch name derived from story slug, or uncommitted changes). If declined, clear `{story_key}` and fall through to instruction 2.
     - **Multiple `review` stories:** **If `{autopilot_mode}` = true, automatically select the first review story and proceed.** Otherwise, present them as numbered options alongside a manual choice option. Wait for user selection. If the user selects a story, set `{story_key}` to the selected story's key and use the selected story's context to determine the diff source as in the single-story case above, and proceed to instruction 3. If the user selects the manual choice, clear `{story_key}` and fall through to instruction 2.
...
4. **If `{autopilot_mode}` = true:** Automatically set `{spec_file}` to the story file path (the story in review status), set `{review_mode}` = `"full"`, and skip to instruction 5. **Otherwise:** Ask the user: **Is there a spec or story file that provides context for these changes?**
...
**If `{autopilot_mode}` = true, skip this checkpoint entirely and proceed to NEXT.**
