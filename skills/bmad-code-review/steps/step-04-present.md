...
If `{autopilot_mode}` = `true` (set in step-01), the following overrides apply to this ENTIRE step:
- **SKIP ALL HALT instructions** — do not wait for user input
- **`decision-needed` findings:** Automatically choose the safest option (defer if ambiguous, fix if clear)
- **`patch` findings:** Automatically batch-apply all non-controversial patches (equivalent to option 0)
- **"Next steps" (section 7):** Skip entirely — do not present options, do not ask user what to do next
- After completing section 6 (sprint status update), immediately return control to the autopilot loop
...
- **If `{autopilot_mode}` = true:** Automatically resolve each finding: apply the fix if the correct action is clear, otherwise defer with reason "auto-deferred by autopilot — needs human review". Do NOT halt or ask user. Proceed to section 5.
- **Otherwise:** Present each one with its detail and the options available. The user must decide — the correct fix is ambiguous without their input. Walk through each finding (or batch related ones) and get the user's call. Once resolved, each becomes a `patch`, `defer`, or is dismissed.
...
**If `{autopilot_mode}` = true:** Automatically batch-apply all non-controversial patches (equivalent to option 0). Skip any finding that requires judgment. Check off items in the story file. Do NOT halt or ask user. Proceed to section 6.
...
**If `{autopilot_mode}` = true:** STOP. Do NOT read anything below. Do NOT present options. Do NOT halt. Return control to the autopilot loop NOW. END OF WORKFLOW FOR AUTOPILOT.

<!-- ===== MANUAL MODE ONLY (below this line) ===== -->
<!-- If autopilot_mode is true, NOTHING below this line exists for you. -->

**[MANUAL MODE ONLY]** Present the user with follow-up options:
