...
- **Step 1 (Determine target story):** If no story specified, automatically pick the first `backlog` story from sprint-status.yaml — do NOT ask user to choose.
- **Step 6 (Finalize):** Skip the "Report completion" output and "Next Steps" suggestions — do NOT suggest running dev-story manually. After saving the story file and updating sprint-status, immediately return control to the autopilot loop.
...
<step n="6" goal="Update sprint status and finalize">
  ...
  <!-- AUTOPILOT: skip all output below -->
  <check if="{autopilot_mode} == true">
    <action>STOP HERE. Do NOT output the story details or next steps below.</action>
    <action>Return control to the autopilot loop. It will run dev-story automatically.</action>
    <action>END OF STEP 6 FOR AUTOPILOT MODE.</action>
  </check>
