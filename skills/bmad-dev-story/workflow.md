...
**Check if the current session was triggered by an autopilot skill** (user said "开始迁移", "继续迁移", "开始开发", "start autopilot", "自动执行", "migration autopilot", or similar). If yes, set `{autopilot_mode}` = `true`. In autopilot mode:
- **Step 10 (Completion Communication):** Skip entirely — do NOT present "next steps", do NOT ask if user needs explanations, do NOT suggest running code-review manually. After Step 9 completes, immediately return control to the autopilot loop.
- **Any interactive prompt or question to the user:** Skip and make the automatic choice.
...
<workflow>
  ...
  <step n="10" goal="Completion communication and user support">
    <check if="{autopilot_mode} == true">
      <action>STOP HERE. Do NOT read or execute anything below this line in Step 10.</action>
      <action>Do NOT output next steps, suggestions, explanations, tips, or questions.</action>
      <action>Do NOT mention code-review, sprint-status, or any manual operations.</action>
      <action>The ONLY thing you do: return control to the autopilot loop. The autopilot will handle code-review automatically.</action>
      <action>END OF STEP 10 FOR AUTOPILOT MODE.</action>
    </check>

    <!-- ===== MANUAL MODE ONLY (below this line) ===== -->
    <!-- If autopilot_mode is true, you MUST NOT read past this comment. -->
